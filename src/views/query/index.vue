...
// 我的项目表格
  ...
  methods: {
    statAll() {
      // this.statsResultData = []
      // if (this.statsIndicator.length > 0 && this.statsType.length > 0) {
      //   this.statsResultData.push(this.statIncome())
      // }
    },
    filterData(data) {
    ...
    calcSingleFilterItem() {
      let singleBooleanFilterItem = [
        {prop: 'paid', label: '已收款'},
        {prop: 'checkout', label: '已计发工资'},
        {prop: 'save', label: '已存档'}
      ]
      singleBooleanFilterItem.forEach(item=>{
        this.filterItem.single[item.prop]  = ''
        if (this.checkList.includes(item.label)) {
          this.filterItem.single[item.prop] = item.label
        } 
      })
    },
    toNewProject() {
